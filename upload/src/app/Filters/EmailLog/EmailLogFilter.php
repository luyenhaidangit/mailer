...
class EmailLogFilter extends FilterBuilder
{
    ...
    public function openCount($count = null)
    {
        $this->builder->when($count, function (Builder $builder) use ($count) {
            $builder->where('open_count','>', 0);
        });
    }

    public function clickCount($count = null)
    {
        $this->builder->when($count, function (Builder $builder) use ($count) {
            $builder->where('click_count','>', 0);
        });
    }
