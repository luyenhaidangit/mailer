...
class SubscriberImport implements SubscriberImportContract
{
        ...
        return $this->import->readImported($this->file, function ($row) use ($metas) {
            foreach ($this->columns as $key => $field) {
                if (in_array($field, ['first_name', 'last_name', 'email']))
                    $metas[$field] = $field == 'email' ? strtolower(trim($row[$key])) : $row[$key];
                else
    ...
    public function sanitize($subscribers)
    {
        ...
        $existedEmails = $this->existedEmails($subscribers);
        $subscribers = $subscribers->whereNotIn('email', $existedEmails);
        ...
        return [
            ...
            'sanitized' => $sanitized->filter(fn($data) => $data['email'] != '')->values(),
            'columns' => $this->columns,
            'existed' => $existedEmails
        ];
    }
    ...
    private function filterDuplicates(Collection $subscribers)
    {
        ...
        return $subscribers->whereIn('email', $duplicateEmails)->map(function ($subscriber) {
