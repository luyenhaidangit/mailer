...
class SubscriberRepository extends AppRepository
{
     ...
     * @return int
     */
    public function chunkImported($subscribers, $isWithCF, \Closure $callback, $lists)
    {
        $count = 0;
        $meta = [
        ...
        foreach ($chunks as $chunk) {
            if ($isWithCF) {
                $callback(array_merge($chunk, $meta), $lists);
                $count++;
            }
            else {
                $callback($this->fillChunks($chunk, $meta), $lists);
                $count += count($chunk);
            }
        }
        return $count;
