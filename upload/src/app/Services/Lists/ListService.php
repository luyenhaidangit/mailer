...
use App\Jobs\Sync\AllSubscriberSync;
...
class ListService extends AppService
{
    ...
    public function save($options = [])
    {
        ...
        if(request('all_subscribers')){
            $count = Subscriber::query()
                ->select('id','status_id')
                ->where('status_id', resolve(StatusRepository::class)->subscriberSubscribed())
                ->count('id');

            $this->syncAllSubscribers($count < 20000);
        }

        if (count(request('subscribers', [])) && !request('all_subscribers'))
    ...
    public function syncAllSubscribers($now = false)
    {
        if ($now){
            AllSubscriberSync::dispatchSync($this->model);
        }else{
            AllSubscriberSync::dispatch($this->model);
        }
    }
