...
use App\Jobs\Import\SubscriberImportWithSync;
...
class SubscriberService extends AppService
{
    ...
    public function saveImported($subscribers, $isWithCF, $existedEmails = []): int
    {
        if ($isWithCF) {
            $this->cfService->setCurrentCustomFields(brand()->id);
        }

        $lists = Lists::query()->whereIn('id', explode(',', request('list')))->get();
        if ($lists->count() && count($existedEmails)){
            $existed_subscribers = Subscriber::query()->whereIn('email', $existedEmails)->pluck('id');
            $lists->each(function (Lists $list) use ($existed_subscribers){
                $list->subscribers()->attach($existed_subscribers);
            });
        }

        return $this->repository->chunkImported($subscribers, $isWithCF, function ($chunk, Collection $lists) use($isWithCF) {

            if ($isWithCF) {
                $subscriber = $this->model::create($chunk);
                if ($lists->count()) {
                    $subscriber->lists()->attach($lists->pluck('id'));
                }
                ...
                if ($lists->count()) {
                    SubscriberImportWithSync::dispatch($chunk, $lists)->onQueue('default');
                } else {
